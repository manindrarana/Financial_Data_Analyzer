"""
Train an XGBoost model for BTC 1h direction prediction.

Queries gold_crypto_features from DuckDB, applies the same stationarity
transformations used in predictor.py, creates a chronologically-split
train/test set, trains XGBoost with the GridSearchCV best parameters
from the BTC_xgboost notebook, and saves the model JSON to
src/models/BTC_1h_xgboost_model.json.

Also saves BTC_1h_xgboost_metadata.json containing the training cutoff
...
META_PATH = os.path.join("src", "models", "BTC_1h_xgboost_metadata.json")
...
def _make_stationary(df):
    ...
    Raw indicators (sma_7, ema_12, macd, atr_14, etc.) are transformed into
    relative measures (distance from close or percentage of close) so the
    model learns from normalized patterns rather than absolute price levels.
    """
    df = df.copy()
...
def main():
    print("=== Training BTC 1h XGBoost Model ===\n")

    print("[1/5] Loading BTC 1h features from gold_crypto_features...")
    ...
    print("[4/5] Splitting train/test (80/20 chronological, no shuffle)...")
    ...
    print("\n[5/5] Training XGBoost with GridSearchCV best params from notebook...")
    print("   Best params: lr=0.05, max_depth=3, n_estimators=100, subsample=1.0")
    ...
    print(f"   Notebook baseline: 52.60% (same params, pre-split parquet)")
    ...
    metadata = {
        ...
        "source": "GridSearchCV from BTC_xgboost.ipynb (3-fold CV, 36 combos)",
    }
